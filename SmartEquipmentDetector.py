...
class SmartEquipmentDetector:
        ...
        try:
            ...
            for i in range(blocks_node.Count):
                try:
                    ...
                    if record_type:
                        print(f"DEBUG: {block_name}의 Record Type: {record_type}")
    ...
    def _get_record_type(self, aspen_application, block_name: str) -> str:
        ...
        try:
            ...
            if record_type_node:
                ...
                print(f"DEBUG: {block_name} -> RECORD_TYPE 노드에서 발견: {record_type}")
                return record_type
            ...
            if block_type_node:
                block_type = block_type_node.Value
                print(f"DEBUG: {block_name} -> BLOCK_TYPE 노드에서 발견: {block_type}")
                return block_type
            ...
            try:
                ...
                if hasattr(block_node, 'BlockType'):
                    block_type = block_node.BlockType
                    print(f"DEBUG: {block_name} -> BlockType 속성에서 발견: {block_type}")
                    return block_type
            except Exception as e:
                print(f"DEBUG: {block_name} -> BlockType 속성 확인 실패: {e}")
            
            # 방법 4: 블록의 하위 노드들을 확인하여 타입 추정
            try:
                block_node = aspen_application.Tree.FindNode(f"\\Data\\Blocks\\{block_name}")
                if block_node:
                    # Input 폴더의 내용 확인
                    input_path = f"\\Data\\Blocks\\{block_name}\\Input"
                    input_node = aspen_application.Tree.FindNode(input_path)
                    if input_node:
                        for i in range(input_node.Count):
                            try:
                                child_node = input_node.Item(i)
                                child_name = child_node.Name
                                if 'TYPE' in child_name.upper() or 'RECORD' in child_name.upper():
                                    if hasattr(child_node, 'Value'):
                                        value = child_node.Value
                                        print(f"DEBUG: {block_name} -> {child_name}에서 발견: {value}")
                                        return value
                            except:
                                continue
            except Exception as e:
                print(f"DEBUG: {block_name} -> 하위 노드 확인 실패: {e}")
            
            print(f"DEBUG: {block_name} -> Record Type을 찾을 수 없음")
            return None
    ...
    def _classify_by_record_type(self, block_name: str, record_type: str) -> Dict[str, Any]:
        ...
        print(f"DEBUG: {block_name}의 Record Type '{record_type}' 분류 시도")
        ...
        for mapping in sorted(self.record_type_mappings, key=lambda x: x.priority, reverse=True):
            for rt in mapping.record_types:
                rt_upper = rt.upper()
                # 정확한 매칭 또는 부분 문자열 매칭
                if (rt_upper == record_type_upper or 
                    rt_upper in record_type_upper or 
                    record_type_upper in rt_upper or
                    any(word in record_type_upper for word in rt_upper.split('-')) or
                    any(word in record_type_upper for word in rt_upper.split('_'))):
                    print(f"DEBUG: {block_name} -> {record_type} -> {mapping.type.value} (매칭: {rt})")
        ...
        print(f"DEBUG: {block_name} -> {record_type} -> 매칭되는 Record Type 없음")
        return None
